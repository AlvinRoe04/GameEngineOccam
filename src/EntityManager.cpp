...
CollisionType EntityManager::CheckCollisions() const {
    for (int i = 0; i < entities.size() - 1; i++) {
        ...
        if (thisEntity->HasComponent<Collider>()) {
            Collider* thisCollider = thisEntity->GetComponent<Collider>();
            for (int j = i + 1; j < entities.size(); j++) {
                auto& thatEntity = entities[j];
                if (thisEntity->name.compare(thatEntity->name) != 0 && thatEntity->HasComponent<Collider>()) {
                    Collider* thatCollider = thatEntity->GetComponent<Collider>();
